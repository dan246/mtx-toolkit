...
def mock_httpx():
    ...
    with patch('httpx.get') as mock_get, patch('httpx.post') as mock_post, \
         patch('httpx.patch') as mock_patch, patch('httpx.delete') as mock_delete:
        yield {
            'get': mock_get,
            'post': mock_post,
            'patch': mock_patch,
        ...
        'items': [
            {
                'name': 'test/stream1',
                'ready': True,
                'source': {'type': 'rtspSource', 'id': 'rtsp://192.168.1.100:554/stream1'},
                'bytesReceived': 1000000
            },
            {
                'name': 'test/stream2',
                'ready': False,
                'source': {'type': 'rtspSource', 'id': 'rtsp://192.168.1.100:554/stream2'},
                'bytesReceived': 0
            },
            {
                'name': 'test/ondemand',
